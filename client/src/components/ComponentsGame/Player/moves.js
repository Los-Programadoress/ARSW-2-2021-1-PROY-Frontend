...
export function changeDirectionSingle(state, { direction }) {
...
// Cambia la dirección del movimiento del jugador dado su indice
export function changeDirectionMulti(state, index, { direction }) {
  const newPlayers = state.players;
  const oldPolarity = setPolarity(state.players[index].direction);
  const newPolarity = setPolarity(direction);
  if (oldPolarity.coord3 === newPolarity.coord3) {
    newPlayers[index] = {
      ...state.players[index],
...
export function walkingSingle(state, { time = Date.now() } = {}) {
...
export function updatePlayer(state, time) {
  ...
  newPlayer.score = state.score + scoreDelta;
...
// Actualiza el estado del movimiento de los jugadores al tiempo
export function walkingMulti(state, { time = Date.now() } = {}) {
  const timeSeconds = (time - state.walkingTime) / 1000;
  if (state.lost) {
    return state;
  }

  const playersWalking = updatePlayerMulti(
    { ...state, walkingTime: time },
    timeSeconds
  );

  return playersWalking;
}

// Cambia el estado del juego; el vector de movimiento de los jugadores y el estado de las pastillas en el tablero
export function updatePlayerMulti(state, time) {
  const newPlayers = state.players;
  const tabs = state.tabs.slice();

  for (var i = 0; i < state.players.length; i++) {
    const newVector = getNewPlayerVector(state.players[i], time);
    const eatentabsIndex = getEatentabs(
      state.tabs,
      state.players[i],
      newVector.position
    ...
    const newPlayer = state.players[i];
    newPlayer.score = state.players[i].score + scoreDelta;
    ...
    state.scoreTeamOne = state.players[0].score + state.players[2].score;
    state.scoreTeamTwo = state.players[1].score + state.players[3].score;

    if (state.scoreTeamOne + state.scoreTeamTwo === 20) {
      state.gameOver = true;
    }
  }
